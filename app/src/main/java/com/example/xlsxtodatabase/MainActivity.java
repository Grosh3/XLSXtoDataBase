...
/* 060326 функционал для сохранения фрагментов схем в базу данных в бинарном формате BLOB.
Передаем в DBrepository список обьектов которые будут содержать в себе три обьекта(соответствующие
каждой папке), в каждом обьекте будет содержаться имя папки, список наименований файлов.
, выполняем поиск по БД элементов колонки 2 (name_eng) из списка, если совпадение найдено,
то получаем массив байт найденного изображения, и записываем его в колонку БД которая соответствует переданной
папке. Если совпадений не найдено то название файла добавляется в массив, который в конце цикла выведется в log.
